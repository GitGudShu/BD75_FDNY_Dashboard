...
OUTPUT_FIG = PROJECT_ROOT / "output" / "figures" / "operational_v3" # Updated folder name
...
def load_data():
    ...
    cols_fire = ["date_key", "hour", "incident_type_key", "dispatch_time", "travel_time", 
                 "engines_assigned_quantity", "other_units_assigned_quantity", "ladders_assigned_quantity"]
    
    # We still need EMS for the Sankey raw data check, but loading Fact for completeness
...
def analyze_reality_gap_sankey(project_root):
    print("Running Reality Gap (Sankey) - MISCLASSIFICATIONS ONLY...")
    
    try:
        # Load raw EMS data
        ems_path = project_root / "data" / "raw" / "EMS.csv"
        if not ems_path.exists():
            print("Warning: EMS.csv raw file not found. Skipping Sankey.")
            return

        ems_raw = pd.read_csv(ems_path, usecols=["INITIAL_CALL_TYPE", "FINAL_CALL_TYPE"])
        
        # --- THE FIX IS HERE ---
        # 1. Filter: Keep ONLY rows where the Initial type DOES NOT MATCH the Final type
        # We don't care about the 1,000,000 times they were right. We care about the errors.
        mismatches = ems_raw[ems_raw["INITIAL_CALL_TYPE"] != ems_raw["FINAL_CALL_TYPE"]].copy()
...
def main():
    try:
        ...
        analyze_reality_gap_sankey(PROJECT_ROOT)
